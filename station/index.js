...
const targetUrl = "http://localhost:3001/station_change";
...
function DataAboutStation() {
  ...
  if (charge_down % 5 == 0 && charge_down != 0 && stat.direction != 8) {
    // заряд машины теряется на каждые 50 запросов
    --stat.charge;
    charge_down = 0;
  } else {
    ++charge_down;
...
app.listen(PORT, () => {
  console.log(`Station server started at http://localhost:${PORT}`);
});
